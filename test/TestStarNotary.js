...
it("can add the star name and star symbol properly", async () => {
  ...
  const tokenId = 6;
  const starName = "Favorite Star";
  const instance = await StarNotary.deployed();
  await instance.createStar(starName, tokenId, {
    from: accounts[0],
  });
  //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
  assert.equal(await instance.name.call(), "HousStarNotary");
  assert.equal(await instance.symbol.call(), "HSNC");
});

it("lets 2 users exchange stars", async () => {
  const instance = await StarNotary.deployed();
  // 1. create 2 Stars with different tokenId
  const star1Name = "My Favorite Star 1";
  const star1TokenId = 7;

  const star2Name = "My Favorite Star 2";
  ...
  assert.equal(await instance.ownerOf.call(star1TokenId), user2);
  assert.equal(await instance.ownerOf.call(star2TokenId), user1);
});
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  const starFound = await instance.lookUptokenIdToStarInfo(starId);

  // 3. Verify if you Star name is the same
  assert.equal(starFound, starName);
});
